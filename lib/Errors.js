...
export class IncompatibleSignatureBug extends TwikiBug {
  ...
  constructor(step, instruction) {
    const message = `I attempted to parse the step ${step.id} using an instruction with an incompatible signature "${instruction.signature.template}" defined in "${instruction.library.name}"`;
