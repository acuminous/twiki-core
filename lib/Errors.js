...
export class IncompatibleInstructionBug extends TwikiBug {
  static code = `${Package.name}/instruction/001`;

  constructor(instruction, step) {
    const message = `I attempted to execute an incompatible instruction ${instruction.appellative} when interpretting the step ${step.appellative}`;
    super(message, IncompatibleInstructionBug.code);
  }
}
