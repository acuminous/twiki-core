...
export default class Signature {
  ...
  constructor({ template, regexp, instruction }) {
  ...
  bind(instruction) {
    return new Signature({ template: this._template, regexp: this._regexp, instruction });
  }
  ...
  parse(step) {
    ...
    if (!match) throw new IncompatibleSignatureBug(step, this._instruction);
